...
class SANBertNetwork(nn.Module):
    def __init__(self, opt, bert_config=None, initial_from_local=False):
        ...
        if initial_from_local:
            self.bert = model_class(self.preloaded_config)
        else:
            #self.bert = model_class.from_pretrained(opt['init_checkpoint'], return_dict=False)
            self.bert = model_class.from_pretrained(opt['init_checkpoint'])
        hidden_size = self.bert.config.hidden_size
        fea_dim = opt.get('fea_dim', 0)
        hidden_size += fea_dim
        ...
        for task_id in range(len(task_def_list)):
            ...
            #elif task_type in [TaskType.TokenRegression, TaskType.TokenClassification]:
            #    out_proj = TokenClasifier(hidden_size, lab, opt)
                ...
                if decoder_opt == 1:
                    ...
                    out_proj = nn.Linear(hidden_size, lab)
            if opt['encoder_type'] == EncoderModelType.DEBERTA or opt['encoder_type'] == EncoderModelType.ELECTRA:
                out_proj = nn.Sequential(LinearPooler(hidden_size), out_proj)
    ...
    def encode(self, input_ids, token_type_ids, attention_mask):
        ...
        if self.opt['encoder_type'] == EncoderModelType.DEBERTA or \
            self.opt['encoder_type'] == EncoderModelType.ELECTRA:
            pooled_output = outputs[0]
    ...
    def embed_forward(self, embed, attention_mask=None, output_all_encoded_layers=True):
        ...
        if self.opt['encoder_type'] == EncoderModelType.DEBERTA:
        ...
        sequence_output = encoder_outputs[0]
        if self.opt['encoder_type'] == EncoderModelType.ELECTRA:
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, premise_mask=None, hyp_mask=None, fea=None, token_maks=None, task_id=0, fwd_type=0, embed=None):
        if fwd_type == 2:
        ...
        #if fea is not None:
        #    pooled_output = torch.cat([pooled_output, fea], dim=1)
        #if task_obj is not None:
        ...
        if task_type == TaskType.Span:
            ...
            logits = self.scoring_list[task_id](sequence_output)
            return logits
        #elif task_type in [TaskType.TokenClassification, TaskType.TokenRegression]:
        #    sequence_output = self.dropout_list[task_id](sequence_output)
        #    logits = self.scoring_list[task_id](sequence_output, token_maks)
        #    return logits
        #elif task_type == TaskType.MultiRC:
        #    pooled_output = self.dropout_list[task_id](pooled_output)
        #    logits = self.scoring_list[task_id](pooled_output)
        #    return logits
        else:
